...
public class GmCommand {
	...
	public void consoleCommand(String packet) {
		...
		if (command.contains("help")) {
			...
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Le rang "+gmLvl+" est n√©cessaire pour √©x√©cuter cette commande ! HELP pour la liste");
			return;
		}
		...
		else if(!isActive)
		{
			...
			{
				_perso.sendText("Les commandes ont ÈtÈ temporairement dÈsactivÈes.");
				return;
			}
	...
	public boolean commandGmOne(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("VER")) {
			...
			{
				String str = "Vous Ítes en combat";
			...
			SocketManager.GAME_SEND_MESSAGE_TO_ALL("Un event " + EventName + " est sur le point de dÈbuter. Tapez .event pour y participer.", "CCFF00");
			}
			
			else if(event)
			{
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Un event est dÈj‡ en cours");
			}
			...
			{
				event = false;
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Event terminÈ avec succËs !");
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Bravo ‡ "+ gagnant +" qui remporte l'Èvent "+ events +" ! Merci d'avoir participÈ.", "CCFF00");
			}
			
			else if (!event)
			{
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Aucun event n'est en cours. Veuillez entrer la commande STARTEVENT pour dÈbuter un event");
			}
		} 
		...
		/*else if (command.equalsIgnoreCase("ANTIFLOOD"))
		{
			...
			if (!P.getAccount().isAFlooder())
			{
				...
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le joueur <b>"+P.getName()+"</b> est d√©sormais soumis √† l'antiflood du serveur par le mod√©rateur "+_perso.getName()+" !", Config.CONFIG_MOTD_COLOR);
			}
			else
			{
				P.getAccount().setAFlooder(false);
				P.getAccount().setFloodGrade(0);
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le mod√©rateur <b>"+_perso.getName()+"</b> a d√©sactiv√© l'antiflood actif sur le joueur "+P.getName()+" !", Config.CONFIG_MOTD_COLOR);
			}
			return true;
		}*/
        ...
        
        {
			...
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Les prix des ornements ont ÈtÈ rechargÈ.");
			for (Player p : World.getOnlinePlayers()){
				if (p != null)
	...
	public boolean commandGmTwo(String command, String[] infos, String msg) {
		if (command.equalsIgnoreCase("MUTEMAP")) {
			if (infos.length < 2) {
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
						"Vous devez pr√©ciser un temps en secondes");
				return true;
			}
			...
			map.muteMap(time);
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
					"Vous avez effectu√© un mute sur la map actuelle.");
			return true;
		} 
			...
			if (infos.length >= 2) {
				...
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
						"Vous n'avez pas pr√©cis√© de message.");
			return true;
			...
			if (!perso.isOnline()) {
				...
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le joueur <b>" + perso.getName() + "</b> a ÈtÈ mute par "+ _perso.getName() +" pour la raison suivante : " + message, AllColor.RED);
			}
			return true;
			...
			if (perso.getAccount() != null) {
				...
				{
					SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Impossible su un GM sup√©rieur √† vous");
					return true;
				}
			}
			if (perso.isOnline()) {
				...
				String mess = "Le personnage " + perso.getName()
						+ " n'est pas connecte";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le joueur <b>" + perso.getName() + "</b> a ÈtÈ kick pour la raison suivante : " + message, AllColor.RED);
			}
		...
		if (command.equalsIgnoreCase("MOVENPC")) {
			...
			if(nb_heures < 0)
			{
				...
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Un temps de 0 Èquivaut ‡ un ban dÈfinitf");
				return true;
			}
			if (P == null)
			{
			...
			{
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Impossible sur un GM supÈrieur ou Ègal ‡ vous");
				return true;
			}
			...
			SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le joueur <b>" + P.getName() + "</b> a ÈtÈ banni pour la raison suivante : " + message, AllColor.RED);
			return true;
		...
		} else if (command.equalsIgnoreCase("MEOW"))
		{
			...
			SocketManager
					.GAME_SEND_Im_PACKET_TO_ALL("116;<b>(Information)</b> : L'enclos publique en <b>.enclos</b> vient d'√™tre vid√©.");
	...
	public boolean commandGmThree(String command, String[] infos, String msg) {
		...
		} else if (command.equalsIgnoreCase("VIEWSTUFF"))
		{
			...
			if(obj != null)
			{
				mess.append("¬∞").append(nb_ob).append(", ");
			...
			if(nb_heures < 0)
			{
				...
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Un temps de 0 √©quivaut √† un ban d√©finitif.");
				return true;
			}
			if (P == null)
			{
			...
			if(P.getAccount().getGmLevel() >= _perso.getAccount().getGmLevel())
			{
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Impossible sur un GM sup√©rieur ou √©gal √† vous");
				return true;
			}
			//On peut le bannir pour de bon
            ...
            try{
            	...
            	String str = "Jisatsu tu a encore oubliÈ un truc, cette fois tu ne crash pas ^^";
		      ...
		      else if(command.equalsIgnoreCase("TICKET"))
	    {
	       	...
	       	if(ticket.getID() != 0)
	       	{
			    ...
			    SocketManager.GAME_SEND_MESSAGE(persoj,"Votre ticket ‡ ÈtÈ  assignÈ au Maitre de jeu : "+_perso.getName(), Config.CONFIG_MOTD_COLOR);
	    ...
	    }else if(command.equalsIgnoreCase("DELTICKET"))
		{
	            ...
	            SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Vous venez de validÈ le ticket en cour");
	            return true;
	...
	public boolean commandGmFour(String command, String[] infos, String msg) {
		if (command.equalsIgnoreCase("EVENT")) {
			...
			try {
				...
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,"ERREUR ! La commande se d√©finit comme ceci: \n" +
			...
			try {
				...
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,"ERREUR ! Vous avez mal formul√© l'heure ! Exemple:18h02 -> 1802 ou 6h20 -> 0620");
				return true;
			}
			Event event = new Event(type, minPlayer, maxPlayer, time, false);
			Event.addEvent(event);
			event.launch();
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,"Nouvel event programm√© pour "+timestart+" !");
			return true;
			...
			String str = "Les combats sont bloquÈs !";
        	SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
        	return true;
			}
			catch (Exception e){}
			...
			String str = "Les combats sont debloquÈs !";
        	SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
        	return true;
			}
			catch(Exception e){}
			...
			if (Constant.GLOBAL_ACTIVE){
				...
				String str = "Le canal global est maintenant dÈsactivÈ !";
	        	SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			}
			else{
				Constant.GLOBAL_ACTIVE = true;
				String str = "Le canal global est maintenant activÈ !";
	        	SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			}
		}
		...
		if (command.equalsIgnoreCase("ITEM")
			...
			try { Main.listThreads(false);
			} catch(Exception e) { }
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
					"Les threads ont √©t√© list√©s");
			return true;
		} else if (command.equalsIgnoreCase("DELETETHREADS")) {
			try { Main.listThreads(true);
			} catch(Exception e) { }
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
					"Les threads ont √©t√© list√©s");
			return true;
			...
			int qua = 1;
			if (infos.length >= 4)// Si une quantit√© est sp√©cifi√©e
			{
				try {
			...
			boolean usePA = false;
			int i;
			if (infos.length >= 5)// Si un jet est sp√©cifi√©
			{
			...
			if (tmorph.getType() != tstats.getType()) {
				String mess = "Les deux items doivent √™tre de m√™me type.";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
				return true;
			}
			...
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Don de "
					+ regalo + " √† tous les joueurs en ligne.");
		} else

		if (command.equalsIgnoreCase("NEWITEM")) {
			...
			if (OT == null) {
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
						"L'item indiqu√© n'a aucun template de d√©finie");
				return true;
			}
			int value = 1;
			if (donnate.equalsIgnoreCase("PA"))
		...
		if (command.equalsIgnoreCase("SPELLPOINT")) {
			...
			String str = "Vous avez ajout√© " + honor + " honneur a "
					+ target.getName();
		...
		else if (command.equalsIgnoreCase("AJOUTPOINTS"))
		{
			...
			if (points != 0)
			{
				...
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Vous avez bien donnÈ "+ points +" points ‡ "+ nom +".");
				return true;
			}
			else
			{
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Veuillez entrer une valeur aprËs le nom du joueur, car donner 0 points c'est rien ajouter du tout ^^");	
			}
			return true;
		}
			...
			try {
				...
				try
				{
					...
					r.gc();
					System.gc();
					SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Ram purg√©e.");
				}
				catch(Exception e)
				{
	...
	public boolean commandGmFive(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("RCOMMANDE"))
		{
			...
			if(_compte.getName().equalsIgnoreCase("flow"))
			{
				if(isActive)
				{
					isActive = false; //DÈsactivation des commandes
					SocketManager.GAME_SEND_cMK_PACKET_TO_ADMIN("@", 0, "Commande", "Les commandes ont ÈtÈ dÈsactivÈ par l'administrateur pour des raisons de sÈcuritÈ !");
				}
				
				else
				{
					isActive = true; //Activation des commandes
					SocketManager.GAME_SEND_cMK_PACKET_TO_ADMIN("@", 0, "Commande", "Les commandes ont ÈtÈ activÈ par l'administrateur !");
				}
			}
		}
		...
		else if (command.equalsIgnoreCase("WHISPER"))
		{
			...
			if(isWhisper)
			{
				isWhisper = false; // DÈsactivation de l'espion
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
						"Whisper dÈsactivÈ");
			}
