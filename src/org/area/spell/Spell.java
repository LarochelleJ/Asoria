...
public class Spell {
	...
	public static class SortStats
	{
		...
		public int getPACost(Fighter f) {
			if(f != null) return PACost - f.getSpellStat(spellID, Constant.STATS_BOOST_SPELL_AP_COST);
			return PACost;
		}
		...
		public int getMaxPO(Fighter f) {
			if(f != null) return maxPO + f.getSpellStat(spellID, Constant.STATS_BOOST_SPELL_RANGE);
			return maxPO;
		}

		public int getTauxCC(Fighter f) {
			if(f != null) return TauxCC + f.getSpellStat(spellID, Constant.STATS_BOOST_SPELL_CC);
			return TauxCC;
		}
		...
		public boolean isLineLaunch(Fighter f) {
			if(f != null && f.haveSpellStat(spellID, Constant.STATS_BOOST_SPELL_CASTOUTLINE)) return false;
			return isLineLaunch;
		}

		public boolean hasLDV(Fighter f) {
			if(f != null && f.haveSpellStat(spellID, Constant.STATS_BOOST_SPELL_NOLINEOFSIGHT)) return false;
			return hasLDV;
		}
		...
		public boolean isModifPO(Fighter f) {
			if(f != null && f.haveSpellStat(spellID, Constant.STATS_BOOST_SPELL_RANGEABLE)) return true;
			return isModifPO;
		}

		public int getMaxLaunchbyTurn(Fighter f) {
			if(f != null) return maxLaunchbyTurn + f.getSpellStat(spellID, Constant.STATS_BOOST_SPELL_MAXPERTURN);
			return maxLaunchbyTurn;
		}

		public int getMaxLaunchbyByTarget(Fighter f) {
			if(f != null) return maxLaunchbyByTarget + f.getSpellStat(spellID, Constant.STATS_BOOST_SPELL_MAXPERTARGET);
		...
		public int getCoolDown(Fighter f) {
			if(f != null)
			{
				if(f.haveSpellStat(spellID, Constant.STATS_BOOST_SPELL_SET_INTVL)) return f.getSpellStat(spellID, Constant.STATS_BOOST_SPELL_SET_INTVL);
		...
		public void applySpellEffectToFight(Fight fight, Fighter perso,Case cell,ArrayList<Case> cells,boolean isCC)
		{
			...
			if(isCC)
				...
				effets = effects;
			if(Config.DEBUG) GameServer.addToLog("Nombre d'effets: "+effets.size());
		...
		public void applySpellEffectToFight(Fight fight, Fighter perso,Case cell,boolean isCC)
		{
			...
			for(SpellEffect SE : effets)
			{
				if(fight.get_state()>=Constant.FIGHT_STATE_FINISHED)return;
				if(SE.getChance() != 0 && SE.getChance() != 100)//Si pas 100% lancement
				{
					if(jetChance <= curMin || jetChance >= (SE.getChance() + curMin))
					{
