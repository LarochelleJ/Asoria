...
public class SpellEffect {
    ...
    private void applyEffect_109(Fight fight)//Dommage pour le lanceur (fixes)
    {
        if (turns <= 0) {
            ...
            if (!isBegin)
                finalDommage = applyOnHitBuffs(finalDommage, caster, caster, fight, poison);//S'il y a des buffs sp�ciaux
        ...
        if (turns <= 0) {
            for (Fighter target : cibles) {
                ...
                if (cibles.get(0).isState(Constant.ETAT_PORTEUR)) {
                    ...
                    cibles.get(0).setState(Constant.ETAT_PORTEUR, 0);
                    f.set_holdedBy(null);
                    cibles.get(0).set_isHolding(null);
                    f.get_fightCell().addFighter(f);
        ...
        if (isCaC)//CaC Terre
        {
            ...
            for (Fighter target : cibles) {
                ...
                } else if (chance > 0 && spell == 108 && !isBegin)//Esprit f�lin ?
                {
                    ...
                    if (!isBegin)
                        fDommage = applyOnHitBuffs(fDommage, caster, caster, fight, poison);//S'il y a des buffs sp�ciaux
