...
public class SpellEffect
	{
		...
		public boolean isSameSpellEffect(SpellEffect se){
			if (effectID == se.effectID) {
				return true;
			}
			return false;
		}
		...
		public int getMaxMinSpell(Fighter cible, int value) {
			...
			if (caster.hasBuff(781)) // Poisse, on regarde si le lanceur a le buff et on minimise ses effets @Flow 
				value = Integer.parseInt(args.split(";")[0]);
			return value;
		}
			...
			for(int id : Constant.ON_HIT_BUFFS)
			{
				for(SpellEffect buff : target.getBuffsByEffectID(id))
				{
					...
					case 79://chance �ca
						...
						//	finalDommage -= renvoie; On passe � la version officielle du sort ^^'
		...
		public void applyToFight(Fight fight,Fighter acaster, ArrayList<Fighter> cibles, boolean isCaC)
		{
			...
			switch(effectID)
			{
				...
				break;
				case 952: // Dissipation
					applyEffect_952(fight);
				break;
		...
		private void applyEffect_952(Fight fight){
			if (fight.get_type() != Constant.FIGHT_TYPE_PVM || fight.dissipationHasBeenLaunch) return;
			for(Fighter target : fight.getFighters(7))
			{
				if (!target.isDead() && target.getTeam() == caster.getTeam()){
                target.addBuff(128, 2, 2, 1, false, spell, "2;-1;-1;3;0;0d0+3", caster, false);
				//Gain de PM pendant le tour de jeu
				if(target.canPlay() && target == caster) target.setCurPM(fight, target.getPM()+3);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 128, caster.getGUID()+"", target.getGUID()+","+3+","+2);
				target.setState(6, 2);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getGUID()+"", target.getGUID()+","+6+",2");
				target.addBuff(950, -1, 2, 1, false, spell, "-1;-1;6;2;0", target, false);
				fight.dissipationHasBeenLaunch = true;
				}
			}
		}
		...
		private void applyEffect_671(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					if(!isBegin)val = applyOnHitBuffs(val,target,caster,fight, poison);//S'il y a des buffs sp�ciaux
		...
		private void applyEffect_672(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				int finalDommage = applyOnHitBuffs(dgt,target,caster,fight, poison);//S'il y a des buffs sp�ciaux
		...
		private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			if(isBlocked > 0)
			{
				// On v�rifie si il va toucher au pi�ge @Flow
				...
				caster.getPersonnage().sendText("Distance A: "+ distanceA +"");*/
		...
		private void applyEffect_106(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				try
				{
				if (target.getMob().getTemplate().getID() == 282){ // Arbre sadida fix @Flow
				caster.getPersonnage().arbreEnModeRenvoi = true;
				caster.getPersonnage().arbreEnQuestion = target;
				}
				}catch(Exception e){}
			}
		}
		...
		private void applyEffect_144(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					if(!isBegin)dmg = applyOnHitBuffs(dmg,target,caster,fight, poison);//S'il y a des buffs sp�ciaux
					if (target.getMob() != null){
						if (target.getMob().getTemplate().getID() == 2750){
							int heal = (int)(-dmg)/2;
							if((caster.getPDV()+heal) > caster.getPDVMAX())
		...
		private void applyEffect_127(ArrayList<Fighter> cibles, Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					if(retrait > 0)
					{
						if(spell == 907) //@Flow - Glyphe d'immobilisation sur mob
						{
						target.setCurPM(fight, target.getCurPM(fight)-retrait);
						}
		...
		private void applyEffect_109(Fight fight)//Dommage pour le lanceur (fixes)
		{
			if(turns <= 0)
			{
				...
				if(!isBegin)finalDommage = applyOnHitBuffs(finalDommage,caster,caster,fight, poison);//S'il y a des buffs sp�ciaux
		...
		private void applyEffect_82(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					if(!isBegin)finalDommage = applyOnHitBuffs(finalDommage,target,caster,fight, poison);//S'il y a des buffs sp�ciaux
		...
		private void applyEffect_5(ArrayList<Fighter> cibles, Fight fight) { //Return Skryn @Flow - Some edits 
			...
			if (spell == 1688) // pi�ge r�pulsif
			{
				if(turns <= 0)
				{
					for(Fighter target : cibles)
					{
						...
						target.get_fightCell().addFighter(target);
						final Fight F = fight;
						final Fighter T = target;
						final int CellID = newCellID;
						target.nbRepulsion++;
						long time = 1100 * target.nbRepulsion;
						Timer timer = new Timer();
						timer.schedule(new TimerTask() {
							  public void run() {
								  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(F, 7, 5, caster.getGUID()+"", T.getGUID()+","+CellID);
							        //SocketManager.GAME_SEND_MESSAGE_TO_ALL(""+ caster.getGUID() +" et target "+ T.getGUID() +" et cellid "+ CellID +"", Config.CONFIG_MOTD_COLOR);
							        T.nbRepulsion--;
							  }
							}, time);
				        //SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 5, caster.getGUID()+"", target.getGUID()+","+newCellID);
				        //SocketManager.GAME_SEND_MESSAGE_TO_ALL(""+ caster.getGUID() +" et target "+ target.getGUID() +" et cellid "+ newCellID +"", Config.CONFIG_MOTD_COLOR);
			...
			if((spell == 368 || spell == 2028 || spell == 151) && turns <= 0)
			{
				for (Fighter cible : cibles) 
				{
					if (cible.isDead())
			...
			if (turns <= 0) 
			{
				//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("nygaaaaaaaaaaa", "0BF9B7");
				for (Fighter cible : cibles) 
				{
					if (cible.isDead())
						continue;
					...
					//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Salut ?", "0BF9B7");
					try {
						Thread.sleep(300);
					} catch (Exception e) {}
					/*
					for (Piege piege : fight.get_traps()) {
						...
						if (dist <= piege.get_size()) {
							piege.onTraped(cible);
							SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Debug #1 Trap", "0BF9B7");
							break;
						}
					}*/
					try{
					for(Piege p : P)
					{
						//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Is it good debug ?", "0BF9B7");
						int dist = Pathfinding.getDistanceBetween(fight.get_map(),p.get_cell().getID(),cible.get_fightCell().getID());
						//on active le piege
						if(dist <= p.get_size()){
							p.onTrapedSournois(cible);
							//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Yea sir ! ^^", "0BF9B7");
						}
					}
					} catch (Exception e) {}
					try {
		...
		private void applyEffect_97(ArrayList<Fighter> cibles,Fight fight, boolean isCaC)
		{
			...
			}else if(turns <= 0)
			{
				...
				for(Fighter target : cibles)
				{
					...
					}else if(chance > 0 && spell==108 && !isBegin)//Esprit f�lin ?
					{
						int fDommage = Formulas.calculFinalDommage(fight,caster, caster,Constant.ELEMENT_TERRE, dmg,false,false,spell,isBegin);
						if(!isBegin)fDommage = applyOnHitBuffs(fDommage,caster,caster,fight, poison);//S'il y a des buffs sp�ciaux
		...
		private void applyEffect_90(ArrayList<Fighter> cibles, Fight fight)
		{
			if(turns <= 0)//Si Direct
			{
				...
				int finalDommage = applyOnHitBuffs(val,caster,caster,fight, poison);//S'il y a des buffs sp�ciaux
		...
		private void applyEffect_108(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			if(turns <= 0)
			{
				...
				for(Fighter target : cibles)
				{
					if((spell == 139 || spell == 1675) && target.getTeam() != caster.getTeam())//Mot d'altruisme && Tourn�e g�n�ral, pas de heal pour ennemi (Skryn, Return ;) )
					{
						continue;
					}
		...
		private void applyEffect_780(Fight fight)
		{
			...
			// On ne r�cup�re pas le dernier mort, mais le premier alli� en fonction du guid
			// Corrig� @Flow
