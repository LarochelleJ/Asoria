...
public class Maps {
	...
	public ArrayList<Player> getPersos()
	{
		...
		synchronized(_persos){persos.addAll(_persos.values());}
		return persos;
		/*try
		{
		    ...
		    for (Entry<Integer, MobGroup> entry : getMobGroups().entrySet())
		    {
		    	try{
			    	if (_persos.size() > 0){
					    ...
					    {
				        	if(z.getValue() == null||z.getValue().getAccount()==null||z.getValue().getAccount().getGameThread()==null||z.getValue().getAccount().getGameThread().getOut()==null)continue;
