...
public class AuctionHouse {
	...
	private class Categorie
	{
		...
		{
			int tempID = toAdd.getObjet().getTemplate(false).getID();
		...
		public boolean delEntry(HdvEntry toDel)
		{
			...
			_templates.get(toDel.getObjet().getTemplate(false).getID()).delEntry(toDel);
			
			if((toReturn = _templates.get(toDel.getObjet().getTemplate(false).getID()).isEmpty()))
				delTemplate(toDel.getObjet().getTemplate(false).getID());
	...
	public class Ligne
	{
		...
		public Ligne(int ligneID, HdvEntry toAdd)
		{
			...
			this.templateID = toAdd.getObjet().getTemplate(false).getID();
	...
	public boolean delEntry(HdvEntry toDel)
	{
		boolean toReturn = _categories.get(toDel.getObjet().getTemplate(false).getType()).delEntry(toDel);
		if(toReturn)
		{
			_path.remove(toDel.getLigneID());
		}
		World.removeHdvItem(toDel.getOwner(), toDel.getHdvID(), toDel);
		SQLManager.SAVE_HDV_ITEM(toDel, false);
		return toReturn;
	}
	...
	public synchronized boolean buyItem(int ligneID,byte amount, int price, Player newOwner)
	{
		...
		try
		{
			...
			if(World.getCompte(toBuy.getOwner()) != null && World.getCompte(toBuy.getOwner()).getCurPlayer() != null)
			{
				SocketManager.GAME_SEND_Im_PACKET(World.getCompte(toBuy.getOwner()).getCurPlayer(),"065;"+price+"~"+toBuy.getObjet().getTemplate(false).getID()+"~"+toBuy.getObjet().getTemplate(false).getID()+"~1");
