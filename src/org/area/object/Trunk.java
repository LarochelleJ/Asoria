...
public class Trunk {
	...
	{	
		...
		{
			SocketManager.GAME_SEND_ECK_PACKET(P.getAccount().getGameThread().getOut(), 5, "");
	...
	public void addInTrunk(int guid, int qua, Player P)
	{
		...
		{
			GameServer.addToLog("Le joueur "+P.getName()+" a tenter d'ajouter un objet dans un coffre qu'il n'avait pas.");
			return;
		}
	...
	public void removeFromTrunk(int guid, int qua, Player P)
	{
		...
		{
			GameServer.addToLog("Le joueur "+P.getName()+" a tenter de retirer un objet dans un coffre qu'il n'avait pas.");
			return;
		}
		if (P.getItems().get(guid).getQuantity() < qua)
		{
	...
	private Item getSimilarTrunkItem(Item obj)
	{
		for(Item value : _object.values())
		{
			if(value.getTemplate(false).getType() == 85)
				continue;
			if(value.getTemplate(false).getID() == obj.getTemplate(false).getID() && value.getStats().isSameStats(obj.getStats()) && value.getStats().getEffect(616161) <= 0)
				return value;
		}
		return null;
	}
