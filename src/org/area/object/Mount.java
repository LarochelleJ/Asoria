...
public class Mount {
	...
	public int getPodsActuels() {
		...
		for (Item obj : _items) {
			if (obj == null)
				continue;
			pods += (obj.getTemplate(true).getPod() * obj.getQuantity());
		}
		return pods;
	}
	...
	public void addObjToSac(int id, int qua, Player perso) {
		Item objToAdd = World.getObjet(id);
		if (objToAdd.getPosition() != -1)
			return;
		Item SameObjInSac = getSimilarObject(objToAdd);
		int newQua = objToAdd.getQuantity() - qua;	
		if(perso.getItems().get(objToAdd.getGuid()) == null)
		{
			GameServer.addToLog("Le joueur "+perso.getName()+" a tenter d'ajouter un objet au store qu'il n'avait pas.");
			return;
		}
		...
		if (SameObjInSac == null) {
			if (newQua <= 0) {
				...
				String str = "O+" + objToAdd.getGuid() + "|" + objToAdd.getQuantity() + "|"
						+ objToAdd.getTemplate(false).getID() + "|" + objToAdd.parseStatsString();
	...
	private Item getSimilarObject(Item obj) {
		for (Item value : _items) {
			ObjTemplate item = value.getTemplate(false);
			if (item.getType() == 85)
				continue;
			if (item.getID() == obj.getTemplate(false).getID() && value.getStats().isSameStats(obj.getStats()) && obj.getStats().getEffect(616161) <= 0)
				return value;
		}
		return null;
	}
	
	public void deleteFromSac(int id, int qua, Player perso) {
		Item objToDelete = World.getObjet(id);
		...
		if (SameObjInInventaire == null) {
			if (newQua <= 0) {
				...
				SameObjInInventaire = Item.getCloneObjet(objToDelete, qua);
				...
				String str = "O+" + objToDelete.getGuid() + "|" + objToDelete.getQuantity() + "|"
						+ objToDelete.getTemplate(false).getID() + "|" + objToDelete.parseStatsString();
				SocketManager.GAME_SEND_EsK_PACKET(perso, str);
			}
		} else {
			if (newQua <= 0) {
