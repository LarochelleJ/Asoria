...
public class Monster {
    ...
    public static class MobGroup {
        ...
        public MobGroup(int Aid, int cellid, String data) {
            id = Aid;
                String[] infos = data.split("/");
                if (infos.length > 8) {
                    align = Integer.valueOf(infos[0]);
                    cellID = cellid;
                    originCellID = cellid;
                    spawnTimeMin = Integer.valueOf(infos[1]);
                    spawnTimeMax = Integer.valueOf(infos[2]);
                    groupD = infos[3];
                    ellaps = Integer.valueOf(infos[4]);
                    if (haveSpawnTime()) {
                        if (haveVariableSpawnTime()) {
                            this.spawnTimeFix = generateRandomSpawnTime();
                        } else {
                    ...
                    aggroDistance = Integer.valueOf(infos[5]);
                    isFix = infos[6] == "1" ? true : false;
                    _creationDate = Long.valueOf(infos[7]);

                    // Mobs
                    int guid = -1;
                    if (infos[8].contains(":")) {
                        String[] mobs = infos[8].split(":");
                        for (int i = 0; i < mobs.length; i++) {
                            String[] mobInfo = mobs[i].split(Pattern.quote("|"));
                            int mobid = Integer.valueOf(mobInfo[0]);
                            int grade = Integer.valueOf(mobInfo[1]);
                            Monster m = World.getMonstre(mobid);
                            if (m != null) {
                                MobGrade mg = m.getGrades().get(grade);
                                _Mobs.put(guid, mg);
                                guid--;
                            }
                        }
        ...
        public String getDataString() {
            // align;spawnTimeMin;spawnTimeMax;groupD;ellaps;aggroDistance;isFix;_creationDate;[_Mobs]
            String data = align + "/" + spawnTimeMin + "/" + spawnTimeMax + "/" + (groupD == null ? "" : groupD) + "/" + ellaps + "/" + aggroDistance + "/" + (isFix ? "1" : "0") + "/" + _creationDate + "/";
            // [_Mobs] = mobID|grade:mobID|grade:
            for (MobGrade MG : _Mobs.values()) {
                data += MG.getTemplate().getID() + "|" + MG.grade + ":";
            }
            return data;
        }
