...
public class Challenge
{
  ...
  public Challenge(Fight fight, int challengeType, int gainXp, int gainDrop)
  {
	...
	public void onFight_end()//Vérifie la validité des challenges en fin de combat (si nécessaire)
	{
	...
	public void show_cibleToPerso(Player p) {
	...
	public void challenge_Foirer(Fighter graceAqui) {
		String nom = "";
		try {
			nom = graceAqui.getPersonnage().getName();
		...
		case 46://Chacun son monstre
			if((caster.getTeam() == 0) && DamagingEffects.contains("|"+effectID+"|"))
			{
				for(Fighter target : targets)
				{
					if(target.getTeam() == 1)
					{
						if(!_arguments.contains(";"+target.getGUID()+",")) _arguments += ";"+target.getGUID()+","+caster.getGUID()+";";
						else if(_arguments.contains(";"+target.getGUID()+",") && !_arguments.contains(";"+target.getGUID()+","+caster.getGUID()+";")) challenge_Foirer(target);
					}
				}
			}
		break;
	...
	public void onMob_die(Fighter mob, Fighter killer) {
		...
		case 44: // Partage
		case 46: // Chacun son monstre
			if(isKiller) _arguments += ";"+killer.getGUID()+";";		
			break;	
	...
	public void onPlayer_endTurn(Fighter fighter)
	{
		...
		switch(_type) {
			...
			case 42://Deux pour le prix d'un
				...
				if(compteur == 2 || compteur == 0) _arguments= "";
				else challenge_Foirer(fighter);
			break;
			default:
			break;
		}
