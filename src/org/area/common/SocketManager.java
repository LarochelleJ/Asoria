...
/**TODO: Fin packets prismes **/
	...
	public static void GAME_SEND_ON_FIGHTER_KICK(Fight f, int guid, int team)
	{
		...
		{
			if(F.getPersonnage() == null || F.getPersonnage().getAccount().getGameThread() == null || F.getPersonnage().getGuid() == guid)continue;
	...
	public static void GAME_SEND_ALTER_FIGHTER_MOUNT(Fight fight, Fighter fighter, int guid, int team, int otherteam)
	{
		...
		{
			if(F.getPersonnage() == null || F.getPersonnage().getAccount().getGameThread() == null || !F.getPersonnage().isOnline())continue;
	...
	{
		String packet = "cMK"+suffix+"|"+guid+"|"+name+"|"+msg;
		for(Player perso : World.getOnlinePlayers())if(perso.isOnline())if(perso.getAccount() != null)if(perso.getAccount().getGmLevel()>0)send(perso,packet);
		if(Config.DEBUG)
			GameServer.addToSockLog("Game: ALL("+World.getOnlinePlayers().size()+"): Send>>"+packet);
	}
