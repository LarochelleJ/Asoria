...
{
	// CANAL GLOBAL
	public static boolean GLOBAL_ACTIVE = true;
	//DEBUG
	public static int DEBUG_MAP_LIMIT 	=	120000;
	//Server
    ...
    public static final int CLASS_STEAMER       =	14;
    public static final int CLASS_ROUBLARD	    =	15;      
	//Sexes
	...
	public static TreeMap<Integer, Character> getStartSortsPlaces(int classID)
	{
		...
		switch(classID)
		{
			...
			break;
			case CLASS_STEAMER:
				start.put(100000,'b');//Flibuste
				start.put(101000,'c');//Ancrage
				start.put(102000,'d');//Harponneuse
			break;
			case CLASS_ROUBLARD:
				start.put(200000,'b');//Détonateur
				start.put(201000,'c');//Extraction
				start.put(202000,'d');//Explobombe
			break;			
		}
		return start;
	}
	
	public static ArrayList<Integer> getSortsClasse(int classID){
		ArrayList<Integer> toReturn = new ArrayList<Integer>();
		switch(classID)
		{
		case CLASS_FECA:
			int[] fecaSpell = { 3,6,17,4,2,1,9,18,20,14,19,5,16,8,12,11,10,7,15,13,1901 };
			for (int i = 0; i < fecaSpell.length; i++){
				toReturn.add(fecaSpell[i]);
			}
		break;
		case CLASS_OSAMODAS:
			int[] osaSpell = { 34,21,23,26,22,35,28,37,30,27,24,33,25,38,36,32,29,39,40,31,1902 };
			for (int i = 0; i < osaSpell.length; i++){
				toReturn.add(osaSpell[i]);
			}
		break;
		case CLASS_SRAM:
			int[] sramSpell = { 61,72,65,66,68,63,74,64,79,78,71,62,69,77,73,67,70,75,76,80,1904 };
			for (int i = 0; i < sramSpell.length; i++){
				toReturn.add(sramSpell[i]);
			}
		break;
		case CLASS_ENIRIPSA:
			int[] eniSpell = { 125,128,121,124,122,126,127,123,130,131,132,133,134,135,129,136,137,138,139,140,1907 };
			for (int i = 0; i < eniSpell.length; i++){
				toReturn.add(eniSpell[i]);
			}
		break;
		case CLASS_ECAFLIP:
			int[] ecaSpell = { 102,103,105,109,113,111,104,119,101,107,116,106,117,108,115,118,110,112,114,120,1906 };
			for (int i = 0; i < ecaSpell.length; i++){
				toReturn.add(ecaSpell[i]);
			}
		break;
		case CLASS_CRA:
			int[] craSpell = { 161,169,164,163,165,172,167,168,162,170,171,166,173,174,176,175,178,177,179,180,1909 };
			for (int i = 0; i < craSpell.length; i++){
				toReturn.add(craSpell[i]);
			}
		break;
		case CLASS_IOP:
			int[] iopSpell = { 143,141,142,144,145,146,147,148,154,150,151,155,152,153,149,156,157,158,160,159,1908 };
			for (int i = 0; i < iopSpell.length; i++){
				toReturn.add(iopSpell[i]);
			}
		break;
		case CLASS_SADIDA:
			int[] sadiSpell = { 183,200,193,198,195,182,192,197,189,181,199,191,186,196,190,194,185,184,188,187,1910 };
			for (int i = 0; i < sadiSpell.length; i++){
				toReturn.add(sadiSpell[i]);
			}
		break;
		case CLASS_XELOR:
			int[] xelSpell = { 82,81,83,84,100,92,88,93,85,96,98,86,89,90,87,94,99,95,91,97,1905 };
			for (int i = 0; i < xelSpell.length; i++){
				toReturn.add(xelSpell[i]);
			}
		break;
		case CLASS_PANDAWA:
			int[] pandaSpell = { 686,692,687,689,690,691,688,693,694,695,696,697,698,699,700,701,702,703,704,705,1912 };
			for (int i = 0; i < pandaSpell.length; i++){
				toReturn.add(pandaSpell[i]);
			}
		break;
		case CLASS_SACRIEUR:
			int[] sacriSpell = { 432,431,434,444,449,436,437,439,433,443,440,442,441,445,438,446,447,448,435,450,1911 };
			for (int i = 0; i < sacriSpell.length; i++){
				toReturn.add(sacriSpell[i]);
			}
		break;
		case CLASS_ENUTROF:
			int[] enuSpell = { 51,43,41,49,42,47,48,45,53,46,52,44,50,54,55,56,58,59,57,60,1903 };
			for (int i = 0; i < enuSpell.length; i++){
				toReturn.add(enuSpell[i]);
			}
		break;
		case CLASS_STEAMER:
	...
	public static TreeMap<Integer,SortStats> getStartSorts(int classID)
	{
		...
		switch(classID)
		{
			...
			break;
			case CLASS_STEAMER:
				 start.put(100000,World.getSort(100000).getStatsByLevel(1));//Flibuste
				 start.put(101000,World.getSort(101000).getStatsByLevel(1));//Ancrage
				 start.put(102000,World.getSort(102000).getStatsByLevel(1));//Harponneuse
				 break;
			case CLASS_ROUBLARD:
	...
	public static boolean isValidPlaceForItem(ObjTemplate template, int place)
	{
		...
		switch(template.getType())
		{
			...
			//Barre d'objets TODO : Normalement le org.area.client bloque les items interdits
	...
	public static void onLevelUpSpells(Player perso,int lvl)
	{
		switch(perso.get_classe())
		{
			...
			break;
			 ...
			 case CLASS_STEAMER:
					if(lvl == 3)
						perso.learnSpell(103000, 1,true,false);//Flibuste
					if(lvl == 6)
						perso.learnSpell(104000, 1,true,false);//Ancrage
					if(lvl == 9)
						perso.learnSpell(105000, 1,true,false);//Harponneuse
					if(lvl == 13)
						perso.learnSpell(106000, 1,true,false);//
					if(lvl == 17)
						perso.learnSpell(107000, 1,true,false);//Gardienne
					if(lvl == 21)
						perso.learnSpell(121000, 1,true,false);//Ressac
					if(lvl == 26)
						perso.learnSpell(109000, 1,true,false);//
					if(lvl == 31)
					...
					if(lvl == 70)
						perso.learnSpell(116000, 1,true,false);//Embuscade
					if(lvl == 80)
						perso.learnSpell(117000, 1,true,false);//
					if(lvl == 90)
						perso.learnSpell(118000, 1,true,false);//
					if(lvl == 100)
				...
				case CLASS_ROUBLARD:
					if(lvl == 3)
						perso.learnSpell(203000, 1,true,false);//Détonateur
					if(lvl == 6)
						perso.learnSpell(204000, 1,true,false);//Extraction
					if(lvl == 9)
						perso.learnSpell(205000, 1,true,false);//Explobombe
					if(lvl == 13)
						perso.learnSpell(206000, 1,true,false);//
					if(lvl == 17)
						perso.learnSpell(207000, 1,true,false);//
					if(lvl == 21)
						perso.learnSpell(208000, 1,true,false);//
					if(lvl == 26)
						perso.learnSpell(209000, 1,true,false);//
					if(lvl == 31)
						perso.learnSpell(210000, 1,true,false);//
					if(lvl == 36)
						perso.learnSpell(211000, 1,true,false);//
					if(lvl == 42)
						perso.learnSpell(212000, 1,true,false);//
					if(lvl == 48)
						perso.learnSpell(213000, 1,true,false);//
					if(lvl == 54)
						perso.learnSpell(214000, 1,true,false);//
					if(lvl == 60)
						perso.learnSpell(215000, 1,true,false);//
					if(lvl == 70)
						perso.learnSpell(216000, 1,true,false);//
					if(lvl == 80)
						perso.learnSpell(217000, 1,true,false);//
					if(lvl == 90)
						perso.learnSpell(218000, 1,true,false);//
					if(lvl == 100)
	...
	public static Stats getMountStats(int color,int lvl)
	{
		...
		switch(color)
		{
			...
			//Armure
			case 88:
				stats.addOneStat(STATS_ADD_RP_AIR, 5);
				stats.addOneStat(STATS_ADD_RP_EAU, 5);
				stats.addOneStat(STATS_ADD_RP_TER, 5);
				stats.addOneStat(STATS_ADD_RP_FEU, 5);
				stats.addOneStat(STATS_ADD_RP_NEU, 5);
				stats.addOneStat(STATS_ADD_VITA, 300);
				stats.addOneStat(STATS_ADD_PERDOM, 100);
				// Olds stats
				/*
				stats.addOneStat(STATS_ADD_RP_AIR, (int)((lvl*12)/100));
				stats.addOneStat(STATS_ADD_RP_EAU, (int)((lvl*12)/100));
				stats.addOneStat(STATS_ADD_RP_TER, (int)((lvl*12)/100));
				stats.addOneStat(STATS_ADD_RP_FEU, (int)((lvl*12)/100));
				stats.addOneStat(STATS_ADD_RP_NEU, (int)((lvl*12)/100));
				stats.addOneStat(STATS_ADD_VITA, lvl*8);
				*/
			break;
			//Balafreux
			case 91:
				...
				stats.addOneStat(STATS_CREATURE, (int)(lvl/25));
			break;
			//Kramkram
			case 92:
				stats.addOneStat(STATS_ADD_VITA, lvl*2);
				stats.addOneStat(STATS_ADD_PROS, (int)(lvl*1));
				stats.addOneStat(STATS_ADD_INIT, lvl*20);
			break;
			//Hurledent
			case 93:
				stats.addOneStat(STATS_ADD_SAGE, lvl*2);
				stats.addOneStat(STATS_ADD_PA, lvl/100);
				stats.addOneStat(STATS_ADD_PO, lvl/100);
			break;
			//Dragouf
			case 94:
				stats.addOneStat(STATS_ADD_PA, 1);
				stats.addOneStat(STATS_ADD_PERDOM, 80);
			break;
			//Dragodinde a plume
			case 95:
				stats.addOneStat(STATS_ADD_PA, lvl/100);
				stats.addOneStat(STATS_ADD_VITA, lvl*3);
				stats.addOneStat(Constant.STATS_ADD_FORC, lvl*1);
				stats.addOneStat(Constant.STATS_ADD_INTE, lvl*1);
				stats.addOneStat(Constant.STATS_ADD_AGIL, lvl*1);
				stats.addOneStat(Constant.STATS_ADD_CHAN, lvl*1);
			break;
			//Sakochere
			case 96:
				stats.addOneStat(STATS_ADD_PM, lvl/100);
				stats.addOneStat(STATS_ADD_PO, lvl/100);
				stats.addOneStat(STATS_ADD_VITA, lvl*3);
				stats.addOneStat(Constant.STATS_ADD_FORC, (lvl*2)/5);
				stats.addOneStat(Constant.STATS_ADD_INTE, (lvl*2)/5);
				stats.addOneStat(Constant.STATS_ADD_AGIL, (lvl*2)/5);
				stats.addOneStat(Constant.STATS_ADD_CHAN, (lvl*2)/5);
			break;
			//Bloupon griotte
            case 97: 
           ...
           //Bloupon indigo
            ...
            //Porcinet
            case 104: 
            	stats.addOneStat(STATS_ADD_INIT, 3500);
            	stats.addOneStat(STATS_ADD_VITA, 100);
            	stats.addOneStat(STATS_ADD_PM, 1);
            	stats.addOneStat(STATS_ADD_PROS, 250);
            	break;
            //Dardargnan
            case 105: 
            	stats.addOneStat(STATS_ADD_SAGE, 50);
            	stats.addOneStat(STATS_ADD_VITA, 500);
            	stats.addOneStat(STATS_ADD_PM, 1);
            	stats.addOneStat(STATS_ADD_PA, 1);
            ...
            */
            ...
            case 75:
            	stats.addOneStat(STATS_ADD_PM, 1);
            	stats.addOneStat(STATS_ADD_SOIN, 20);
            	stats.addOneStat(STATS_ADD_CC, 2);
				stats.addOneStat(STATS_ADD_VITA, 400);
				stats.addOneStat(Constant.STATS_ADD_FORC, 75);
				stats.addOneStat(Constant.STATS_ADD_INTE, 75);
				stats.addOneStat(Constant.STATS_ADD_AGIL, 75);
				stats.addOneStat(Constant.STATS_ADD_CHAN, 75);
            break;
            /*//Dragodinde Blop 1
            case 104:
            	stats.addOneStat(STATS_ADD_PA, lvl/100);
	...
	public static ObjTemplate getParchoTemplateByMountColor(int color)
	{
		switch(color)
		{
			...
			//Balafreux
			case 91: return World.getObjTemplate(7036);
			//Kramkram
			case 92: return World.getObjTemplate(7037);
			//Hurledent
			case 93: return World.getObjTemplate(7038);
			//Dragouf
			case 94: return World.getObjTemplate(7039);
			//Dragodinde a plume
			case 95: return World.getObjTemplate(7040);
			//Sakochere
			case 96: return World.getObjTemplate(7041);
			//Bloupon griotte
			case 97: return World.getObjTemplate(895472);
			//Bloupon coco
			case 101: return World.getObjTemplate(895473);
			//Bloupon indigo
			case 102: return World.getObjTemplate(895474);
			//Bloupon reinette
			case 103: return World.getObjTemplate(895475);
			//Porcinet
			case 104: return World.getObjTemplate(895476);
			//Dardagnan
			case 105: return World.getObjTemplate(895477);
			//Kwaltess
			case 98: return World.getObjTemplate(70043);
			//Bloupon griotte
	...
	public static int getMountColorByParchoTemplate(int tID)
	{
		...
		for(int a = 1;a<106;a++)if(getParchoTemplateByMountColor(a)!=null)if(getParchoTemplateByMountColor(a).getID() == tID)return a; 
		return -1;
	}
	...
	public static int getNearCellidUnused(Player _perso)
	{
		...
		if(_perso.getMap().getCase(_perso.get_curCell().getID()+cellFront).getDroppedItem() == null
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()+cellFront).getPersos().isEmpty()
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()+cellFront).isWalkable(false))
		{
			return cellFront;
		}else
		if(_perso.getMap().getCase(_perso.get_curCell().getID()-cellBack).getDroppedItem() == null
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()-cellBack).getPersos().isEmpty()
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()-cellBack).isWalkable(false))
		{
			return cellBack;
		}else
		if(_perso.getMap().getCase(_perso.get_curCell().getID()+cellRight).getDroppedItem() == null
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()+cellRight).getPersos().isEmpty()
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()+cellRight).isWalkable(false))
		{
			return cellRight;
		}else
		if(_perso.getMap().getCase(_perso.get_curCell().getID()-cellLeft).getDroppedItem() == null
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()-cellLeft).getPersos().isEmpty()
				&& _perso.getMap().GetCases().get(_perso.get_curCell().getID()-cellLeft).isWalkable(false))
		{
			return cellLeft;
		}
