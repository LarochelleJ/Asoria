			...
			if (_fighter.getMob() == null) {
				...
				try {
					...
					case 0:// Ne rien faire
						...
						break;
					case 1:// Attaque, Buff soi-m�me, Buff Alli�s, Avancer
							// vers ennemis.
					...
					case 2:// Soutien
						...
						break;
					case 3:// Avancer vers Alli�s, Buff Alli�s, Buff sois
							// m�me
...
*/
		        ...
		        while (!stop && F.canPlay()) {
		            int attack = 0;
		                ...
		                if (attack != 0)// Il n'y pas d'attaque
		                {
		                    ...
		                    else{ // a combatu on s'enfuit plus rien � faire
		...
		private static void apply_type2(Fighter F, Fight fight) {
			...
			while (must_stop < 2 && F.canPlay()) {
				...
				catch(Exception e){Logs.addToDebug("Interrompu dans le Repos Thread 2 : "+((long) (System.currentTimeMillis()/1000-_startT)));return;}
			}
			}
			catch(InterruptedException ie)
			{
...
/*
		...
		*/
		 ...
		 // @Flow - � r�viser
		 private static boolean testCotes(int cell1, int cell2)//Bord, hors zone. Cell 2 = Cellule o� on va et cell1 = Cellule o� nous sommes
		    {
...
*/
		...
		private static int attackIfPossibleFlow(Fight fight, Fighter fighter)// 0 = Rien, 5 = EC, 666 = NULL, 10 = SpellNull ou ActionEnCour ou Can'tCastSpell, 0 = AttaqueOK
	    {
			/**
			 * @Flow
			 * Cette fonction tiens compte des personnages invisibles, elle va attaquer al�atoire
			 * Malheuresement cette version n'est pas tr�s efficace avec tous les mobs
			 */
	        try {
	            if (fight == null || fighter == null) {
	                return 0;
	            ...
	            if(ennemyList != null && !ennemyList.isEmpty()){
	                for (Entry<Integer, Fighter> t : ennemyList.entrySet()) // pour chaque ennemi on cherche le meilleur sort
	                {
	                	...
	                	//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Sort trouv� !", "0BF9B7");
	                	//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Nom du personnage s�lectionn�: "+ t.getValue().getPersonnage().getName() +"", "0BF9B7");
	                    ...
	                    if(SS != null) // Si il y a un sort
	                    {
	                    	...
	                    	{*/
	                    		...
	                    		//SocketManager.GAME_SEND_MESSAGE_TO_ALL2("Le joueur"+ target.getPersonnage().getName() +" a �t� s�lectionn� comme cible.", "0BF9B7");
		                  break;
	                    	/*}
	                    	else
	                    	{
	                                ...
	                                if(curArea > area){ //si zone plus grande
	                                    ...
	                                    for(Case possibleCell : possibleLaunch){
	                                        /**if(possibleCell.getFirstFighter() != null && possibleCell.getFirstFighter().getTeam2() == fighter.getTeam2()){
	                                            continue; 
	                                            @Flow
	                                            Cette condition emp�che les sorts qui attaques ses alliers, j'ai remarqu� que �a cause un �norme probl�me
		...
		/**
		 * @Flow
		 * Les invocations sont maintenants g�r�es.
		 */
		private static int attackIfPossible(Fight fight, Fighter fighter)// 
		{
					...
					.entrySet()) // pour chaque sort du mob
			{
				int targetVal = getBestTargetZone(fight, fighter, S.getValue(),
						fighter.get_fightCell().getID()); // on d�termine le
															// meilleur
                    ...
                    for(SortStats SS3 : getLaunchableSort(fighter, fight, 0)){
                        ...
                        if(curArea > area){ //si zone plus grande
                            ...
                            for(Case possibleCell : possibleLaunch){
                                ...
                                if(Config.DEBUG){
                                    System.out.println("Sort " + SS.getSpellID() + " s�lectionn�");
                                }
                                break;
                            }
                        }
		...
		private static int calculInfluence(Fight fight, SortStats ss,
			...
			for (SpellEffect SE : ss.getEffects()) {
				...
				num++;
				/** Fin de la d�termination **/
				// System.out.println("SpellEffect : "+SE.getEffectID()+"   Nbr ennemis : "+ennemies+"   Nbr alli�s : "+allies);
				int inf = 0;
		...
		private static SortStats getBestSpellForTarget2(Fight fight, Fighter F,	Fighter T) {//Version optimis�e
			int inflMax = 0;
			SortStats ss = null;
			int curInfl = 0;
