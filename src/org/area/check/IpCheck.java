...
public class IpCheck {
    ...
    private static Map<Integer, IpInstance> _instances = new TreeMap<Integer, IpInstance>();
    ...
    public static class IpInstance {
        ...
        //Connexionx au org.area.game server
        private long _Game_last;
        private int _Game_over;
        //Pakcet par seconde sur le org.area.game server
        ...
        public boolean newRealMConnexion() {
            if (_ip == "127.0.0.1" || _ip == "51.254.116.255") return true;
            if (_banned) {
    ...
    public static IpInstance getInstance(String ip) {
        ip = ip.trim();
        if (ip.isEmpty()) return null;
        for (IpInstance ii : _instances.values()) {
    ...
    public static IpInstance addInstance(String ip) {
        ip = ip.trim();
        if (ip.isEmpty()) return null;
        IpInstance ii = new IpInstance(ip);
